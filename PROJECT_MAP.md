...
### 0.12 Completed User Role Assignment UI — 2026-07-08

تم إنجاز خطوة Frontend محدودة ضمن مرحلة إدارة المستخدمين:

```text
YM-USERS-UI-001B — User Role Assignment UI
```

#### المنجز الفعلي

تم تحويل صفحة المستخدمين من عرض قراءة فقط إلى واجهة إدارة أدوار محدودة وآمنة للمصرح لهم.
...
- إضافة عمود `الإجراءات / Actions` في جدول المستخدمين.
- إضافة زر `إدارة الأدوار / Manage roles` لكل مستخدم.
- فتح modal لإدارة أدوار المستخدم.
- عرض اسم المستخدم والبريد الإلكتروني داخل المودال.
- عرض الأدوار المتاحة كخيارات قابلة للتحديد.
- حفظ التغييرات عبر endpoint الموجود مسبقًا:

```text
PUT /api/admin/users/{user}/roles
```

- تحديث صف المستخدم بعد نجاح الحفظ بدون إعادة تحميل الصفحة كاملة.
- عرض رسالة نجاح بعد الحفظ.
- عرض رسائل خطأ واضحة لحالات 403 و422 والأخطاء العامة.

#### حماية super-admin في الواجهة

- إذا كان المستخدم المستهدف يحمل دور `super-admin` تظهر رسالة تحذيرية واضحة.
- لا تسمح الواجهة بإزالة اختيار `super-admin` من حساب super-admin.
- Backend ما زال هو طبقة الحماية النهائية لقواعد super-admin.
...
ما زالت بعض أسماء CSS/state الداخلية تحمل كلمة `readonly` تاريخيًا، لكن النص الظاهر للمستخدم أصبح صحيحًا ويصف Role Assignment. يمكن تنظيف هذه التسميات لاحقًا ضمن خطوة refactor صغيرة إذا لزم.


### 0.13 Completed Users Roles Column Visual Polish & Role Modal Clarity — 2026-07-08

تم إنجاز خطوة Frontend بصرية محدودة ضمن مرحلة إدارة المستخدمين:

```text
YM-USERS-UI-001C — Users Roles Column Visual Polish & Role Modal Clarity
```

#### المنجز الفعلي

تم تحسين تجربة إدارة الأدوار داخل صفحة المستخدمين بدون تغيير Backend أو API contract.
...
#### تحسينات عمود الأدوار

- استبدال أسماء الأدوار النصية داخل الجدول بأيقونات ملوّنة.
- إضافة أيقونة مميزة لكل دور أساسي:
  - super-admin
  - admin
  - staff
  - client
  - designer
- إضافة fallback بصري للأدوار غير المعروفة.
- إبقاء tooltip باسم الدور عند المرور على الأيقونة.
- تحسين عرض تعدد الأدوار داخل الصف ومنع القص المزعج.

#### تحسين زر إدارة الأدوار

- استبدال الزر النصي بأيقونة أكبر وأوضح.
- إضافة tooltip:
...
- الحفاظ على نفس السلوك الحالي عند الضغط.

#### تحسين نافذة إدارة الأدوار

- تكبير عنوان المودال والنصوص الداخلية.
- تحسين وضوح المستخدم والبريد داخل المودال.
- تحسين شكل خيارات الأدوار وتباينها.
- إضافة أيقونة لكل role option داخل المودال.
- تحسين أزرار الحفظ والإلغاء.
- تحسين الوضوح في الوضعين الفاتح والداكن.
...
- `staff` يحصل على `dashboard.overview.view` للوصول إلى `/admin` بوصفها Dashboard الداخلية الكاملة.
...
- يدخل إلى `/admin` بوصفها Dashboard الداخلية الكاملة عند امتلاك `dashboard.overview.view`.
- لم تعد `/staff` الصفحة الرئيسية الكاملة للموظف، ويمكن أن تبقى صفحة مختصرة/قديمة أو مساحة فريق منفصلة لاحقًا.
- تظهر له روابط `/admin/*` الموجودة فقط عند امتلاك الصلاحيات الدقيقة المرتبطة بها.
- لا يظهر له `/admin/staff`، إذ يبقى Staff Management الحالي محصورًا في `super-admin`.
...
- يبقى رابط وسطح `/admin/staff` الحاليان خاصين بـ `super-admin` إلى أن تُضاف صلاحيات `admin.staff.*`.
...
- `staff` section يبقى خاصًا بـ `super-admin` حتى إضافة صلاحيات `admin.staff.*`.
- الأقسام التشغيلية placeholder غير المبنية لا تُفتح لغير `super-admin` عبر `dashboard.overview.view`، وتبقى مؤجلة حتى بناء مصادرها وصلاحياتها الدقيقة.
...
إضافة صلاحيات دقيقة لأقسام Dashboard التشغيلية عند بناء مصادرها وفتحها لغير super-admin
```
...
### 0.17 Completed Internal Dashboard Permission-Scoped Surfaces — 2026-07-11

تم إغلاق وربط نموذج Dashboard الداخلي الحالي عبر البحث العام، وأسطح Dashboard Overview، وActivity Feed، وتوجيه `/admin`، وروابط Sidebar المبنية على الصلاحيات.
...
7bd149c feat: add permission scoped dashboard search
b4c4891 feat: connect topbar to dashboard search
e3989ec test: lock dashboard search permission model
bd2e29f feat: scope internal dashboard surfaces by permission
90be252 fix: scope dashboard activity feed by permission
```

#### Canonical internal Dashboard route

أصبحت `/admin` صفحة Dashboard الداخلية الكاملة للأدوار الداخلية التالية:
...
- `super-admin` يملك رؤية وتحكمًا شاملين في الأسطح الحالية.
- `admin` و`staff` يدخلان `/admin` عند امتلاك `dashboard.overview.view`.
- يتم توجيه `staff` إلى `/admin` بدل عزله في `/staff` بوصفها صفحته الرئيسية الكاملة.
- تبقى `/staff` صفحة مختصرة/قديمة حاليًا، ويمكن أن تتحول لاحقًا إلى مساحة فريق منفصلة، لكنها ليست محور Dashboard الداخلي الكامل.
- يبقى `client` و`designer` وguest خارج `/admin` حتى إذا مُنحوا صلاحيات `dashboard.*` أو `admin.*` بالخطأ.

#### Permission-scoped Dashboard Overview surfaces

يعيد `/api/dashboard/overview` الأسطح التالية حسب الصلاحيات الدقيقة للمستخدم الداخلي، وتبني صفحة `/admin` عرضها من الاستجابة نفسها:
...
ويُبنى فلتر الأقسام من `sections` المسموح بها نفسها، لذلك تتزامن البطاقة والرسم والقسم في الفلتر تلقائيًا عند إضافة الصلاحية المناسبة إلى دور داخلي.

| السطح | شرط الظهور لغير `super-admin` |
|-------|--------------------------------|
| `overview` | `dashboard.overview.view` |
| `users` | `admin.users.view` |
| `roles` | `admin.roles.view` |
| `permissions` | `admin.permissions.view` |
| `access` | `admin.access.view` |
| `staff` | لا يظهر حاليًا لغير `super-admin` |

لم يعد اسم الدور `admin` سببًا كافيًا لإظهار كل الأقسام، كما لم يعد اسم الدور `staff` سببًا لحجب قسم يملك الموظف صلاحيته الدقيقة.

#### Permission-scoped Sidebar visibility

يستخدم Sidebar نموذج المستخدم الداخلي نفسه، ويعرض الروابط الموجودة وفق القواعد التالية:

| الصلاحية أو الشرط | الرابط |
|--------------------|--------|
| `dashboard.overview.view` أو `super-admin` | `/admin` |
| `admin.users.view` أو `super-admin` | `/admin/users` |
| `admin.roles.view` أو `super-admin` | `/admin/roles` |
| `admin.permissions.view` أو `super-admin` | `/admin/permissions` |
| `super-admin` فقط حاليًا | `/admin/staff` |

- لا تُعرض فواصل فارغة عندما لا يملك المستخدم روابط داخل مجموعة معينة.
- تبقى روابط content وinsights المرتبطة بوحدات غير مبنية محصورة في `super-admin` حاليًا.
- لم تُضف مسارات جديدة لهذا النموذج؛ ظهور الروابط محصور في المسارات الموجودة.

#### Search integration

يبقى Dashboard Search داخل TopBar permission-scoped وفق القسم `0.16`: يستطيع المستخدم الداخلي البحث عند امتلاك `dashboard.overview.view`، ولا تظهر له نتائج users/roles/permissions إلا بصلاحياتها الدقيقة، بينما تبقى نتائج staff لـ `super-admin` فقط.

تحتفظ `/admin/users` ببحثها السياقي الخاص، ولا تتحول إلى dropdown البحث العام.

#### Permission-scoped Activity Feed

بعد `90be252` أصبح Activity Feed داخل `/api/dashboard/overview` جزءًا من نموذج Dashboard permission-scoped، ولم يعد ظهوره معتمدًا على اسم الدور العام `admin` أو `staff`.

تُصنّف النشاطات الحالية حسب مصدرها، ويُفتح كل مصدر بصلاحيته الدقيقة:

| مصدر النشاط | الصلاحية المطلوبة لغير `super-admin` | نوع النشاط |
|-------------|----------------------------------------|------------|
| `users` | `admin.users.view` | `user-created` |
| `roles` | `admin.roles.view` | `role-created` |
| `permissions` | `admin.permissions.view` | `permission-created` |

- يرى `admin` و`staff` نشاطات المصدر فقط عند امتلاك الصلاحية المرتبطة به.
- لا يرى المستخدم الداخلي نشاطات مصدر لا يملك صلاحيته، وتكون `activities` فارغة إذا لم يملك أيًا من صلاحيات المصادر الحالية.
- يرى `super-admin` جميع أنواع النشاط المتاحة حاليًا.
- لا تُنشأ نشاطات placeholder أو fake/demo، وتبقى العناصر مشتقة من سجلات users/roles/permissions الموجودة فعليًا.
- يبقى الدمج والترتيب الزمني والحد الأقصى الحالي البالغ ثمانية عناصر محفوظًا دون تغيير في عقد الاستجابة.
- يبقى `client` و`designer` خارج Dashboard الداخلي، ولا يمنحهما إسناد صلاحيات Dashboard/Admin بالخطأ وصولًا إلى Activity Feed.

#### Visual verification

أكد الفحص البصري من واجهة إدارة الأدوار أن إضافة صلاحيات Dashboard/Admin إلى دور `staff` تؤدي تلقائيًا إلى ظهور الأسطح المرتبطة بها في `/admin`:

- البطاقة.
- الرسم.
- القسم داخل فلتر الأقسام.
- الرابط الموجود داخل Sidebar، عندما يكون للمسار رابط مبني حاليًا.

#### Deferred boundaries

- تبقى placeholder sections ووحدات content/insights غير المبنية خاصة بـ `super-admin` أو مؤجلة حتى اكتمال مصادرها وصلاحياتها الدقيقة.
- لا يفتح هذا الإنجاز Staff Management لغير `super-admin`، ولا يضيف صلاحيات `admin.staff.*`.
- لا يغير هذا القرار ترتيب خطط المراحل التشغيلية القادمة أو مصادر البيانات المؤجلة.


### 0.18 Planned Dashboard Reports & Analytics Hardening — 2026-07-11

هذا القسم يثبت خطة التحسين التالية بعد إغلاق نموذج Dashboard الداخلي permission-scoped، ولا يمثل تنفيذًا حاليًا أو تغييرًا في المعمارية المعتمدة.

#### Current state

النظام الحالي:

- منظم بصريًا ويقدم بطاقات ورسومًا وفلاتر وأسطحًا مترابطة داخل `/admin`.
- يقيّد sections/cards/charts/activities والبحث وروابط Sidebar حسب الصلاحيات الدقيقة.
- يعرض خيارات `day/week/month/year` ويحافظ على الفترة المختارة في response، لكنه لا يطبق بعد فلترة زمنية حقيقية كاملة على الأرقام.
- يعرض رسومًا مبنية على نقاط الملخص الحالية، وليست Timeseries زمنية حقيقية كاملة.
- لا يملك بعد قرار نطاق نهائيًا لنظام reports المستقبلي أو عقد reports module.

#### Required execution order

يتم تنفيذ الخطوات التالية بالتسلسل فقط، ولا تُجمع في دفعة تنفيذ واحدة:

| الترتيب | المهمة | النطاق المعتمد |
|---------|--------|----------------|
| 1 | `YM-DASH-PERIOD-METRICS-001` | تطبيق فلترة `day/week/month/year` حقيقية في Backend على أرقام Dashboard. |
| 2 | `YM-DASH-TIMESERIES-CHARTS-001` | إنتاج series زمنية حقيقية للرسوم اعتمادًا على بيانات Backend المفلترة. |
| 3 | `YM-DASH-CHART-TOOLTIP-DETAILS-001` | تحسين تفاصيل tooltip للرسوم والبطاقات بما يطابق القيم والفترات الحقيقية. |
| 4 | `YM-DASH-QUICK-STATS-PERMISSION-SCOPE-001` | جعل quickStats يعرض فقط المؤشرات المسموحة فعليًا للمستخدم الداخلي. |
| 5 | `YM-REPORTS-SCOPE-DECISION-001` | تحديد نطاق نظام التقارير المستقبلي وعقوده وحدوده قبل بناء reports module. |

#### Step completion gate

لا يبدأ تنفيذ أي خطوة لاحقة قبل إغلاق الخطوة السابقة عبر:

1. اختبارات مستهدفة تثبت السلوك الجديد وعدم كسر نموذج الصلاحيات.
2. `build` ناجح عند وجود تغيير يحتاج بناء Backend أو Frontend.
3. فحص بصري موثق عند وجود تغيير UI.
4. commit مستقل وواضح للخطوة ثم push قبل فتح المهمة التالية.

#### Planning boundaries

- تُنفذ كل مهمة كنطاق مستقل صغير وقابل للتحقق.
- لا يتم تنفيذ الخطوات الخمس دفعة واحدة.
- لا تتغير معمارية Dashboard الحالية أو عقود الصلاحيات المعتمدة لمجرد إضافة هذه الخطة.
...
### 0.20 Audit and Tracking First Decision — 2026-07-11

هذا القرار يحدّث ترتيب التنفيذ المقترح بعد اعتماد نطاق Reports وAnalytics، دون إلغاء الفصل الوظيفي الموثق بينهما. لا تبدأ المرحلة القادمة ببناء الواجهات الكاملة، بل بتأسيس مصدر منظم وآمن للأحداث التي ستحتاجها التقارير والتحليلات لاحقًا.

#### Architectural decision

- لا يتم بناء صفحات Reports وAnalytics الكاملة مباشرة في المرحلة الحالية.
- لا يؤجل تسجيل الأحداث والتدقيق إلى حين اكتمال جميع وحدات المنصة.
- يعتمد مسار وسط يبدأ ببناء أساس Audit / Tracking، ثم يستخدم هذا الأساس لاحقًا ضمن Reports وAnalytics عند بناء واجهاتهما.
...
#### Meaning of “track everything”

المقصود بتسجيل كل شيء هو تسجيل كل حدث مهم، ومفيد، وقابل للمراجعة أو التدقيق. لا يعني ذلك نسخ كل request أو payload دون تمييز.

- يكون السجل منظمًا، وآمنًا، وقابلًا للبحث، وله نوع حدث وفاعل وهدف ونتيجة ووقت وسياق منقح عند الحاجة.
- لا تسجل كلمات المرور أو التوكنات أو الكوكيز أو أسرار الدفع أو مفاتيح الوصول.
- لا تخزن البيانات الحساسة الخام أو payloads كاملة لمجرد توفرها.
- يطبق تقليل البيانات وإخفاء الحقول الحساسة قبل حفظ أي سياق إضافي.
- يحدد نطاق كل حدث والغرض منه ومدة الاحتفاظ به قبل إدخاله في التنفيذ الفعلي.

#### Immediate scope definition

تبدأ الخطوات التالية بتحديد نطاق Audit / Tracking قبل بناء foundation تقني:

- تحديد أنواع الأحداث وأسمائها الموحدة.
- تحديد الأحداث التي يجب تسجيلها والأحداث التي يجب استبعادها.
- تحديد شكل السجل الأساسي والحقول الآمنة والقابلة للبحث.
- تحديد العلاقة بين الفاعل والهدف والنتيجة والسياق المنقح.
- تحديد ضوابط الخصوصية والاحتفاظ والوصول إلى السجلات.

بعد اعتماد النطاق، يبنى foundation تدريجيًا لتسجيل:

- تسجيل الدخول والخروج.
- محاولات تسجيل الدخول الفاشلة.
- إنشاء المستخدمين والموظفين.
- تعديل إسناد الأدوار والصلاحيات.
- إنشاء الأدوار والصلاحيات المخصصة وتعديلها وحذفها.
- زيارات صفحات الإدارة المهمة.
- عمليات البحث داخل Dashboard.
- أخطاء الوصول 401 و403.
- الأحداث التشغيلية الحساسة عند بناء الطلبات والأعمال والمحفظة والمدفوعات.

#### Deferred work

يؤجل إلى مراحل لاحقة:

- بناء Reports UI الكاملة.
- بناء Analytics UI الكاملة.
- بناء snapshots أو aggregations.
- بناء export إلى PDF أو Excel.
- بناء التحليلات العميقة والمقارنات طويلة المدى.
- ربط وحدات المنصة التشغيلية التي لم تُبنَ بعد.

#### Relationship with Reports and Analytics

- تقرأ Reports لاحقًا من الجداول الأصلية للحصول على تفاصيل السجلات، ومن audit/activity logs للحصول على الأحداث والتغييرات الإدارية.
- تستخدم Analytics لاحقًا aggregations أو snapshots عندما تصبح المقارنات بعيدة المدى أو الاستعلامات المباشرة ثقيلة.
- لا يستبدل Audit / Tracking صفحات Reports أو Analytics، بل يوفر لهما مصدر بيانات مهمًا وقابلًا للمراجعة.
- تبقى الجداول الأصلية مصدر الحقيقة لبيانات المجال، بينما يوثق سجل التدقيق من فعل ماذا ومتى وما نتيجة الحدث.

#### Proposed execution sequence

يصبح ترتيب التنفيذ المقترح:

1. `YM-AUDIT-EVENTS-SCOPE-001`
2. `YM-AUDIT-EVENTS-FOUNDATION-001`
3. `YM-AUDIT-CURRENT-MODULES-001`
4. `YM-ADMIN-PAGE-VIEWS-TRACKING-001`
5. العودة إلى `YM-REPORTS-USERS-MVP-001`
6. ثم `YM-ANALYTICS-USERS-MVP-001`

#### Decision boundaries

- هذا القسم يوثق قرارًا معماريًا فقط.
- لا يضيف جداول أو migrations.
- لا يضيف APIs.
- لا يضيف صفحات أو مكونات واجهة.
...
- رابط إدارة الأعمال الرئيسي هو `/admin/works`.
- لا تظهر روابط الأقسام الفرعية دائمًا في القائمة الجانبية العامة؛ تظهر فقط عند دخول المستخدم إلى `/admin/works` أو أي مسار داخله.
- تظهر الأقسام الفرعية كتوسعة مرئية داخل Sidebar الحالي وتحت رابط "الأعمال" نفسه، وليست روابط عامة مستقلة ودائمة.
...
- **النظرة العامة:** إحصائيات الأعمال، وملخص قيد المراجعة، والمنشور والمخفي والمميز، والبلاغات، وآخر الإجراءات.
- **كل الأعمال:** قائمة جميع الأعمال، والبحث والفلاتر والفرز، وتفاصيل العمل ومعاينته وملفاته، والإجراءات الفردية والجماعية، والتصنيفات والوسوم، وربط العمل بالمصمم، وأسباب الرفض وطلبات التعديل، وسجل مصغر داخل التفاصيل.
- **طلبات المراجعة:** حالات `submitted` و`in_review` و`changes_requested`، وفتح العمل وبدء مراجعته واعتماده، وطلب تعديل، ورفض مع سبب، والنشر بعد الاعتماد عند سماح الصلاحية، وسجل المراجعة.
- **الظهور والتمييز:** الأعمال المنشورة والمخفية والمميزة والمثبتة، وإجراءات النشر والإخفاء والتمييز وإزالته والتثبيت وإلغائه، وإدارة الظهور العام.
- **البلاغات والمخالفات:** الأعمال المبلّغ عنها، وعدد البلاغات ونوع البلاغ وحالته، ومراجعة البلاغ، وإخفاء العمل أو رفضه أو طلب تعديله، وتجاهل البلاغ، واستعادة العمل، وأرشفة البلاغ، وسجل البلاغات.
- **التصنيفات والوسوم:** التصنيفات والوسوم وعدد الأعمال داخل كل تصنيف، وإضافة التصنيف أو الوسم وتعديله وتعطيله، ونقل الأعمال إلى تصنيف، وتحديث وسوم أعمال محددة.
- **سجل الأعمال:** يعتمد الأحداث `work.created`, و`work.submitted`, و`work.review.started`, و`work.approved`, و`work.changes_requested`, و`work.rejected`, و`work.published`, و`work.hidden`, و`work.featured`, و`work.unfeatured`, و`work.archived`, و`work.restored`, و`work.category.changed`, و`work.tags.updated`, و`work.report.reviewed`.
- **إعدادات وصلاحيات الأعمال:** مصفوفة صلاحيات تغطي العرض والإنشاء والتعديل والمراجعة والاعتماد والرفض وطلب التعديل والنشر والإخفاء والتمييز والأرشفة والاستعادة والإجراءات الجماعية، وإدارة التصنيفات والوسوم والبلاغات، وعرض سجل الأعمال، وإدارة إعدادات الأعمال.

#### Global UX and implementation rules

- يخصص حقل البحث العلوي لسياق كل صفحة، مع فلاتر كاملة ومفهومة وtooltips واضحة.
- تدعم الجداول الترتيب التصاعدي والتنازلي، وترتبط الإحصائيات بالبطاقات والرسوم ذات الصلة.
- تنشأ الصلاحيات على مستوى العمليات والأزرار والحقول الحساسة، وليس على مستوى دخول الصفحة فقط.
- يسجل Audit لكل إجراء مهم وفق أسماء الأحداث المعتمدة وببيانات آمنة.
- يكبر خط البيانات وتحسن قابلية القراءة، وتتميز الأقسام والحالات بصريًا بالألوان المناسبة.
- يمنع `v-html` وconsole logs، ولا تستخدم مكتبات Charts خارجية.
- لا يغلق أي قسم قبل اكتماله وظيفيًا وفق نطاقه المعتمد.
