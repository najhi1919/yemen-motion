<template>
  ...
  >
    ...
    <div class="ym-authoring-actions__buttons">
      ...
      <button
        v-if="mode === 'edit' && showSubmitReview"
        type="button"
        class="is-submit"
        :disabled="submitDisabled || submitting"
        :aria-describedby="submitDisabledReason ? 'ym-submit-disabled-reason' : undefined"
        @click="$emit('submitReview')"
      >
        {{ submitting ? text.submitting : submitLabel }}
      </button>
    </div>
    <small v-if="submitDisabledReason" id="ym-submit-disabled-reason" class="ym-authoring-actions__submit-reason">{{ submitDisabledReason }}</small>
  </aside>
</template>
...
defineEmits<{ save: []; reset: []; back: []; submitReview: [] }>()
const text = computed(() => props.locale === 'ar' ? {
  ...
  dirtyCopy:(count:string)=>`لديك ${count} تغييرات غير محفوظة.`,submitting:'جارٍ الإرسال…'
} : {
  ...
  dirtyCopy:(count:string)=>`You have ${count} unsaved changes.`,submitting:'Submitting…'
})
...
.ym-authoring-actions>div>div{display:grid;min-width:0;gap:2px}.ym-authoring-actions strong{font-size:14px;line-height:1.4}.ym-authoring-actions.is-create-ready strong{color:var(--aw-emerald)}.ym-authoring-actions small{color:var(--aw-muted);font-size:12.75px;line-height:1.45}.ym-authoring-actions__buttons{display:flex;flex:0 0 auto;gap:8px}.ym-authoring-actions button{min-height:44px;border-radius:12px;padding:0 16px;font-size:13.5px;font-weight:850;cursor:pointer}.ym-authoring-actions button:disabled{cursor:not-allowed;opacity:.48}.is-secondary{border:1px solid var(--aw-border);color:var(--aw-text);background:var(--aw-control)}.is-primary{border:0;color:#fff;background:linear-gradient(135deg,var(--aw-violet),var(--aw-magenta));box-shadow:0 10px 22px color-mix(in srgb,var(--aw-violet) 24%,transparent)}.is-submit{border:1px solid color-mix(in srgb,var(--aw-cyan) 45%,var(--aw-border));color:var(--aw-text);background:linear-gradient(135deg,color-mix(in srgb,var(--aw-violet) 18%,var(--aw-control)),color-mix(in srgb,var(--aw-cyan) 12%,var(--aw-control)))}.ym-authoring-actions__submit-reason{flex-basis:100%;color:var(--aw-amber);font-size:12.5px}button:focus-visible{outline:3px solid color-mix(in srgb,var(--aw-electric) 42%,transparent);outline-offset:2px}
