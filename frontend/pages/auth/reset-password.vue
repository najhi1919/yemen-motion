    ...
    <div ref="pageRoot" class="ym-auth-page__content">
      <div class="ym-auth-stack">
        ...
        <GlassLoginCard :state="motionState">
          <div v-if="!hasToken" class="ym-auth-form">
            <div>
              <h1 id="reset-title" class="ym-auth-form__title">إعادة تعيين كلمة المرور</h1>
              <p class="ym-auth-form__subtitle">رابط الاستعادة غير صالح أو منتهي الصلاحية</p>
            </div>
            <div class="ym-auth-links">
              <NuxtLink to="/auth/forgot-password" class="ym-auth-link">طلب رابط جديد</NuxtLink>
              <NuxtLink to="/auth/login" class="ym-auth-link">العودة إلى تسجيل الدخول</NuxtLink>
            </div>
          </div>

          <form v-else class="ym-auth-form" aria-describedby="reset-status" @submit.prevent="submitReset">
            <div>
              <h1 id="reset-title" class="ym-auth-form__title">إعادة تعيين كلمة المرور</h1>
              <p class="ym-auth-form__subtitle">أدخل كلمة المرور الجديدة</p>
            </div>

            <p v-if="message" id="reset-status" class="ym-auth-alert" :class="messageClass" role="alert" aria-live="polite">
              {{ message }}
            </p>
            ...
            <button ref="submitButton" class="ym-auth-submit" type="submit" :disabled="isLoading" aria-label="تغيير كلمة المرور">
              <span v-if="isLoading">جاري تغيير كلمة المرور...</span>
...
useHead({
  title: 'إعادة تعيين كلمة المرور | Yemen Motion',
  meta: [
    { name: 'description', content: 'إعادة تعيين كلمة المرور على منصة Yemen Motion.' }
  ]
})
...
const message = ref('')
const errors = reactive<{ email?: string; password?: string; password_confirmation?: string }>({})
...
async function submitReset() {
  ...
  try {
    ...
    message.value = response?.message || 'تم تغيير كلمة المرور بنجاح.'
    ...
    message.value = errorData?.message || err?.message || 'رابط استعادة كلمة المرور غير صالح أو منتهي الصلاحية.'
    errorShake(pageRoot.value)
    window.setTimeout(() => { motionState.value = 'idle' }, 900)
  }
}
