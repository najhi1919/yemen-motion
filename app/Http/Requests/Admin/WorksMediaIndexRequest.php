...
class WorksMediaIndexRequest extends FormRequest
{
    public function authorize(): bool
    {
        ...
        return $user->hasAnyRole(['admin', 'staff'])
            ...
            && ($user->can('admin.works.media.view')
        ...
        $validator->after(function (Validator $validator): void {
            foreach (array_keys($this->query->all()) as $parameter) {
                $validator->errors()->add(
                    (string) $parameter,
                    'معاملات الاستعلام غير مدعومة لعرض وسائط العمل.',
                );
            }
            ...
            foreach (array_keys($this->all()) as $field) {
                $validator->errors()->add(
                    (string) $field,
                    'جسم الطلب غير مدعوم لعرض وسائط العمل.',
                );
            }
        });
    }
