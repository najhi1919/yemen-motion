...
class DisableWorksTagRequest extends FormRequest
{
    public function authorize(): bool
    {
        return $this->authorizedFor('admin.works.taxonomy.tags.disable');
    }
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            foreach (array_keys($this->query->all()) as $parameter) {
                $validator->errors()->add((string) $parameter, 'معاملات الاستعلام غير مدعومة لتعطيل وسم الأعمال.');
            }

            foreach (array_keys($this->request->all()) as $field) {
                $validator->errors()->add((string) $field, 'تعطيل وسم الأعمال لا يقبل حقولًا في الطلب.');
            }
        });
    }
