...
class UpdateWorksCategoryRequest extends FormRequest
{
    /** @var list<string> */
    ...
    {
        return $this->authorizedFor('admin.works.taxonomy.categories.update');
    }
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            foreach (array_keys($this->query->all()) as $parameter) {
                $validator->errors()->add((string) $parameter, 'معاملات الاستعلام غير مدعومة لتحديث تصنيف الأعمال.');
            }
            ...
            foreach ($unexpectedFields as $field) {
                $validator->errors()->add((string) $field, 'حقل الطلب غير مدعوم لتحديث تصنيف الأعمال.');
            }

            if (array_intersect($bodyFields, self::ALLOWED_BODY_FIELDS) === []) {
                $validator->errors()->add('category', 'يجب إرسال حقل واحد على الأقل لتحديث تصنيف الأعمال.');
            }
        });
    }
