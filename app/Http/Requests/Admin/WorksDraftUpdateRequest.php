...
class WorksDraftUpdateRequest extends FormRequest
{
    /** @var list<string> */
    ...
    private const FIELD_PERMISSIONS = [
        'title' => 'admin.works.update.basic',
        'summary' => 'admin.works.update.basic',
        'description' => 'admin.works.update.basic',
        'media_type' => 'admin.works.update.media',
    ...
    public function rules(WorksSettingsStore $settingsStore): array
    {
        ...
        return [
            'title' => ['sometimes', 'required', 'string', 'min:2', 'max:160'],
        ...
        $validator->after(function (Validator $validator): void {
            foreach (array_keys($this->query->all()) as $parameter) {
                $validator->errors()->add(
                    (string) $parameter,
                    'معاملات الاستعلام غير مدعومة لتحديث مسودة العمل.',
                );
            }

            $bodyFields = array_keys($this->request->all());

            foreach (array_diff($bodyFields, self::ALLOWED_BODY_FIELDS) as $field) {
                $validator->errors()->add(
                    (string) $field,
                    'حقل الطلب غير مدعوم لتحديث مسودة العمل.',
                );
            }
            ...
            if (array_intersect($bodyFields, self::ALLOWED_BODY_FIELDS) === []) {
                $validator->errors()->add(
                    'work',
                    'يجب إرسال حقل واحد على الأقل لتحديث مسودة العمل.',
                );
            }
        });
    }
