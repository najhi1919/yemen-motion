...
class UpdateWorkTagsAssignmentRequest extends FormRequest
{
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            $this->rejectQueryParametersAndUnknownFields($validator, ['tag_ids']);
        });
    }
    ...
    private function rejectQueryParametersAndUnknownFields(Validator $validator, array $allowedFields): void
    {
        foreach (array_keys($this->query->all()) as $parameter) {
            $validator->errors()->add((string) $parameter, 'معاملات الاستعلام غير مدعومة لإسناد وسوم العمل.');
        }

        foreach (array_diff(array_keys($this->request->all()), $allowedFields) as $field) {
            $validator->errors()->add((string) $field, 'حقل الطلب غير مدعوم لإسناد وسوم العمل.');
        }
    }
}
