...
class DesignerWorkUpdateRequest extends FormRequest
{
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            foreach (array_keys($this->query->all()) as $parameter) {
                $validator->errors()->add((string) $parameter, 'معاملات الاستعلام غير مدعومة لتحديث مسودة العمل.');
            }
            $bodyFields = array_keys($this->request->all());
            foreach (array_diff($bodyFields, self::FIELDS) as $field) {
                $validator->errors()->add((string) $field, 'حقل الطلب غير مدعوم لتحديث مسودة العمل.');
            }
            if (array_intersect($bodyFields, self::FIELDS) === []) {
                $validator->errors()->add('work', 'يجب إرسال حقل واحد على الأقل لتحديث مسودة العمل.');
            }
        });
    }
