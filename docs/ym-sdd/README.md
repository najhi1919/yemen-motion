...
يحتوي [`phases/`](phases/README.md) على المرجع التشغيلي طويل العمر لكل مرحلة: الخطة، الحالة الفعلية، القرارات، سجل التقدم، معايير القبول، المحطات، وبوابة الإغلاق.

- `phase`: يجمع نطاقًا مرحليًا واعتمادياته وحالته عبر عدة محطات.
- `spec`: يعرّف عقدًا فنيًا أووظيفيًا قبل التنفيذ.
- `task`: يضبط وحدة تنفيذ محددة وأدلة إغلاقها.
- `ADR`: يسجل قرارًا معماريًا طويل الأثر.

أول مرحلة موثقة بهذا النظام هي [Phase 6 — Designer Profiles](phases/06-designer-profiles/README.md). لا تستبدل طبقة المراحل `PROJECT_MAP.md`، بل تفصل حالته التشغيلية وتربط الخطة بالأدلة.
